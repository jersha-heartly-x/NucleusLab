...
exports.regularSchedule = function (req, res) {
    ...
    db.query(q, (err, result)=> {
        if(err) {
            ...
            const labs = ["CSL1", "CSL2", "CSL3", "DSL", "NSL", "IIL", "OCL", "SKAVA", "SCL", "CL"];
                    ...
                    db.query(q, (err, result)=>{
                        if(err) {
                            ...
                            for(var i=0; i<result.length; i++){
                                ...
                                for(var j = result[i].fromperiod; j <= result[i].toperiod; j++) {
                                    const y = j - 1;
                                    table[x][y] = "blocked";
                                }
...
exports.deleteSchedule = (req, res) => {
    ...
    db.query(sql, (err, result)=>{
        if(err) {
            ...
            sql = `SELECT academic_year, semester from course_dates where '${new Date().toISOString().slice(0, 10)}' BETWEEN start_date and end_date;`;
            ...
            db.query(sql, (err, result)=>{
                if(err) {
                    ...
                    res.render("delete_schedule", {title: "Schedule", menu: "Delete Schedule", defaultOptions :result[0], success: "Deleted successfully!"});
                }
            })
        }
    })
...
exports.checkAvailability = function (req, res) {
    const date = req.body.date,
        ...
        labs = ["CSL1", "CSL2", "CSL3", "DSL", "NSL", "IIL", "OCL", "SKAVA", "SCL", "CL"], 
        table = [];
    ...
    db.query(q, (err, result) => {
        ...
        if(err) {
            ...
            if(result.length != 0 ) {
                ...
                res.render("add_regular_schedule", {title: "Schedule", menu: "Add Schedule", alert: "Already Scheduled!", defaultOptions : { academic_year: academicYear, semester: sem } });
            }
            else {
                ...
                db.query(q, (err, result) => {
                    if(err) {
                        ...
                        if(result.length != 0) {
                            ...
                            res.render("add_regular_schedule", {title: "Schedule", menu: "Add Schedule", alert: "Already Blocked!", defaultOptions : { academic_year: academicYear, semester: sem } });
                        }
                        else {
                            ...
                            res.render("add_regular_schedule", {title: "Schedule", menu: "Add Schedule", success: "Schedule added!", defaultOptions : { academic_year: academicYear, semester: sem } });                          
                        }
                    }
                })
            }
